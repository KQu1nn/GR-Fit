<template>
  ...
  >
    <div class="flex flex-row gap-5 items-center cursor-pointer">
      <p class="font-sans italic text-2xl text-gray-200 font-bold">
        GR FIT <span class="text-red-500/90">Academia</span>
      </p>
    </div>

    <ul
      class="hidden md:flex flex-row gap-10 list-none text-md font-medium text-sm"
    >
      <li
        v-for="item in menu"
        :key="item.id"
    ...
    >
      <span
        class="w-6 h-0.5 bg-white transition"
    ...
    <transition name="slide-fade">
      ...
      >
        <ul
          class="flex flex-col gap-6 text-lg font-medium list-none"
        >
          <li
            v-for="item in menu"
            :key="item.id"
            @click="() => { goToSection(item.id); toggleMenu() }"
